...
description: The Venafi integrated PKI secrets engine for Vault.
---

# Venafi PKI Secrets Engine for HashiCorp Vault

The Venafi PKI Secrets Engine provides applications with the ability to
dynamically generate X.509 certificates. The
[Venafi Trust Protection Platform](https://www.venafi.com/platform/trust-protection-platform)
or [Venafi Cloud](https://www.venafi.com/platform/cloud/devops) back end assures
compliance with enterprise policy and consistency with industry standard trust
protection. Designed with the same interface as the built-in PKI secrets engine,
services can get certificates without manually generating a private key and CSR,
submitting to a CA, and waiting for a verification and signing process to
complete. Venafi's certificate authority integrations and policy controls,
combined with Vault's built-in authentication and authorization mechanisms,
provide the verification functionality.

Like the built-in PKI secrets engine, short-lived certificates for ephemeral
workloads are the primary focus of the Venafi PKI Secrets Engine. As such,
revocation is not currently supported.

The Venafi PKI Secrets Engine makes use of HashiCorp Vault's
...
considerations. Before using Venafi PKI Secrets Engine, you should read every
consideration.

### Trust Protection Platform Requirements
...
Trust Protection Platform's REST API (WebSDK) must be secured with a
...
If you are using Venafi Cloud, be sure to set up a template, project, and any
other dependencies that appear in the Venafi Cloud documentation.  

* Set up a template that includes CA account information. See
[Venafi Cloud Templates](https://docs.venafi.cloud/help/VenafiCloud/Certificates/Issuing_Templates/c-Cloud-issuing-templates-about.htm).
* Create a project or zone that identifies the template and other information.
See [Venafi Cloud Projects](https://docs.venafi.cloud/help/VenafiCloud/Projects/t-Cloud-project-create.htm).

## Setup

Before certificates can be issued, you must complete these steps to configure the
Venafi PKI Secrets Engine:
...
1. Enable the Venafi PKI Secrets Engine:
...
After the Venafi PKI Secrets Engine is configured and a user/machine has a Vault
...
Venafi PKI Secrets Engine uses the same
